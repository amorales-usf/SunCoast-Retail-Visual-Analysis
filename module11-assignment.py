...
def plot_quarterly_sales_trend():
    ...
    ax.set_title("Quarter Sales Trend")
    ax.grid(True)
...
def plot_customer_age_distribution():
    ...
    for ax, loc in zip(axes.flatten(), customer_df['Location'].unique()):
...
def create_business_dashboard():
    ...
    # Your code here
    fig, axes = plt.subplots(2,2)
    axes[0,0].plot(sales_df.groupby('QuarterLabel')['Sales'].sum())
    ...
    axes[1,0].bar(cat_sales.index, cat_sales.values)
    axes[1,1].hist(customer_df['PurchaseAmount'], bins=20)
    return fig
